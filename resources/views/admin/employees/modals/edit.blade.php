            ...
            <form id="editEmployeeForm-{{ $employee->id }}" action="{{ route('employee.update', $employee->id) }}" method="POST">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="sm:col-span-2 space-y-4">
                        <label for="file-{{ $employee->id }}" class="block text-sm font-medium text-gray-900 dark:text-white">Dokumen</label>
                        
                        <!-- Custom file input -->
                        <div class="flex items-center justify-center w-full">
                            <label for="file-{{ $employee->id }}" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 dark:hover:bg-gray-700 dark:bg-gray-700 dark:border-gray-600">
                                ...
                                <input type="file" name="file" id="file-{{ $employee->id }}" class="hidden" accept=".pdf,.doc,.docx" />
                            </label>
                        </div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                            <div class="flex items-center space-x-3">
                                ...
                                <span class="text-sm text-gray-500 dark:text-gray-400">File saat ini: {{ basename($employee->file) }}</span>
                            </div>
                        </div>
                        @endif
                    
                        <!-- Preview container -->
                        <div id="filePreviewContainer-{{ $employee->id }}" class="hidden">
                            <!-- File Preview -->
                            <div id="filePreview-{{ $employee->id }}" class="hidden">
                                <div class="p-4 bg-white rounded-lg border border-gray-200 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p id="fileName-{{ $employee->id }}" class="text-sm font-medium text-gray-900 truncate dark:text-white"></p>
                                            <p id="fileSize-{{ $employee->id }}" class="text-sm text-gray-500 truncate dark:text-gray-400"></p>
                                        </div>
                                        <button type="button" id="removeFile-{{ $employee->id }}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                                            </svg>
                                        </button>
                ...
                <div class="sticky bottom-0 bg-white dark:bg-gray-800 px-6 py-4 border-t border-gray-200 dark:border-gray-600">
                    <div class="flex items-center justify-end space-x-2">
                        <button type="button" data-modal-toggle="editEmployeeModal-{{ $employee->id }}" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-red-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                            Batal
                        </button>
...
document.addEventListener('DOMContentLoaded', function() {
    @foreach($employees as $employee)
    initializeFilePreview({{ $employee->id }});
...
function initializeFilePreview(employeeId) {
    const fileInput = document.getElementById(`file-${employeeId}`);
    const filePreviewContainer = document.getElementById(`filePreviewContainer-${employeeId}`);
    const filePreview = document.getElementById(`filePreview-${employeeId}`);
    const fileName = document.getElementById(`fileName-${employeeId}`);
    const fileSize = document.getElementById(`fileSize-${employeeId}`);
    const removeFile = document.getElementById(`removeFile-${employeeId}`);
